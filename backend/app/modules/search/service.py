...
class SearchService:
    ...
    async def search(self, term: str) -> SearchResponse:
        ...
        jobs, units, drivers, customers, quotations = await asyncio.gather(
            ...
            self._query("jobs", "id, job_number, alat_diangkut, status, customer:customers(nama_perusahaan)",
                        ["job_number", "alat_diangkut", "asal", "tujuan"], cleaned),
            self._query("units", "id, kode_unit, no_polisi, status, jenis_unit:jenis_unit_id(nama)",
                        ["kode_unit", "no_polisi"], cleaned),
            self._query("drivers", "id, nama, no_hp, is_active",
                        ["nama", "no_hp"], cleaned),
            self._query("customers", "id, nama_perusahaan, kota, pic_nama, is_active",
                        ["nama_perusahaan", "kota", "pic_nama"], cleaned),
            self._query("quotations", "id, quote_number, customer_nama, status",
                        ["quote_number", "customer_nama"], cleaned),
        )
        ...
        for r in jobs:
            ...
            hits.append(SearchHit(
                kind="job", id=r["id"], label=r["job_number"],
        ...
        for r in units:
            ...
            hits.append(SearchHit(
                kind="unit", id=r["id"], label=r["kode_unit"],
                sublabel=_join(nama_jenis, r.get("no_polisi")),
                href=f"/units/{r['id']}"))
        for r in drivers:
            hits.append(SearchHit(
                kind="driver", id=r["id"], label=r["nama"],
                sublabel=_join(r.get("no_hp"), None if r.get("is_active", True) else "nonaktif"),
                href=f"/drivers/{r['id']}/edit"))
        for r in customers:
            hits.append(SearchHit(
                kind="customer", id=r["id"], label=r["nama_perusahaan"],
                sublabel=_join(r.get("kota"), r.get("pic_nama")),
                href=f"/customers/{r['id']}/edit"))
        for r in quotations:
            hits.append(SearchHit(
                kind="quotation", id=r["id"], label=r["quote_number"],
                sublabel=_join(r.get("customer_nama"), r.get("status")),
