...
import type { Invoice, SumberDana } from "@/lib/types";
...
export function InvoiceDetailView({ invoice: inv, sumberDana }: Props) {
  ...
  async function ubahStatus(
    ...
    setPending(true);
    const res = await setInvoiceStatusAction(inv.id, status, { alasan });
    setPending(false);
    setBatalOpen(false);
    if (!res.ok) {
  ...
  async function simpanPembayaran() {
    ...
    setPending(true);
    const res = await addInvoicePaymentAction(inv.id, {
  ...
  async function hapusPembayaran(id: string) {
    setPending(true);
    const res = await deleteInvoicePaymentAction(id, inv.id);
    setPending(false);
    setHapusBayar(null);
    if (!res.ok) {
      toast.error(res.error);
