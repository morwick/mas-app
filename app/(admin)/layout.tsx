...
export default async function AdminLayout({
  ...
  const [user, unitsCount, jobsCount, driversCount, notifications] =
    await Promise.all([
      getCurrentUser(),
      activeUnitsCount().catch(() => 0),
      activeJobsCount().catch(() => 0),
      activeDriversCount().catch(() => 0),
      // Lonceng tidak boleh menjatuhkan seluruh halaman kalau salah satu
      // sumbernya bermasalah — panel kosong lebih baik daripada layar error.
