...
class _JobDetailScreenState extends ConsumerState<JobDetailScreen> {
  ...
  void _toast(String msg, {bool error = false}) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Text(msg),
      backgroundColor: error ? MasColors.danger : null,
    ));
  }
          ...
          Padding(
            ...
            child: Row(
              ...
              children: [
                ...
                Flexible(
                  child: Text(reason, style: const TextStyle(fontSize: 12, color: MasColors.muted), textAlign: TextAlign.center),
                ),
              ],
            ),
          ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Expanded(child: Text(job.customerNama, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w700))),
            ...
            _Place(icon: Icons.trip_origin, label: 'Asal', value: job.asal),
            const SizedBox(height: 8),
            _Place(icon: Icons.location_on, label: 'Tujuan', value: job.tujuan),
            const Divider(height: 20),
            _Info('Berangkat', formatDateTime(job.etd)),
            _Info('Estimasi sampai', job.eta == null ? '-' : '${formatDateTime(job.eta)}${job.etaIsEstimated ? ' (estimasi)' : ''}'),
            if (job.routeDistanceKm != null) _Info('Jarak', '${job.routeDistanceKm!.toStringAsFixed(0)} km'),
            if (job.unitKode != null) _Info('Unit', '${job.unitKode}${job.unitNoPolisi != null ? ' • ${job.unitNoPolisi}' : ''}'),
            ...
            children: [
              Container(
                ...
                child: done
                    ...
                    : Center(child: Text('${i + 1}', style: TextStyle(fontSize: 12, color: color, fontWeight: FontWeight.w700))),
              ),
              const SizedBox(height: 4),
              Text(
                s.label,
...
class _Place extends StatelessWidget {
  const _Place({required this.icon, required this.label, required this.value});
  final IconData icon;
  final String label;
  final String value;
...
class _Info extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
        ...
        child: Row(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            SizedBox(width: 110, child: Text(label, style: const TextStyle(fontSize: 13, color: MasColors.muted))),
...
class _SectionTitle extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(
        children: [
          ...
          if (trailing != null) Text(trailing!, style: const TextStyle(color: MasColors.muted, fontWeight: FontWeight.w600)),
        ],
      );
}
