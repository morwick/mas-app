...
import { syncUnitMileage } from "@/features/services/api";
import { ServiceFormModal } from "@/features/services/components/service-form-modal";
import { CalibrateBaselineModal } from "@/features/services/components/calibrate-baseline-modal";
import {
  jenisServiceLabel,
  type ServiceRecord,
...
export function ServiceHistoryTab({ unit, initialRecords }: Props) {
  ...
  useEffect(() => {
    ...
    async function tick() {
      try {
        const body = await syncUnitMileage(unit.id);
        if (cancelled) return;
  ...
  async function handleSync() {
    ...
    try {
      ...
      setLastSyncAt(new Date());
      toast.success(
        `Odometer ter-update (${formatKm(body.km)} hari ini)`
      );
    } catch (e) {
      toast.error(
