...
import { updateJobStatusAction, cancelJobAction } from "@/lib/actions/jobs";
...
export function QuickStatusBar({ jobId, status }: Props) {
  ...
  async function doCancel() {
    setLoading(true);
    const res = await cancelJobAction(jobId, cancelReason);
    setLoading(false);
    // cancelJobAction redirect ke /jobs — kalau berhasil tidak akan return ke sini
    if (res?.ok === false) {
      toast.error(res.error);
    }
  }
