...
import {
  createJenisUnitAction,
  deleteJenisUnitAction,
  updateJenisUnitAction
} from "@/lib/actions/jenis-unit";
...
export function JenisUnitView({ list }: Props) {
  ...
  async function save() {
    ...
    const res =
      editOpen.mode === "new"
        ? await createJenisUnitAction(editOpen.nama)
        : await updateJenisUnitAction(editOpen.id!, editOpen.nama);
    setPending(false);
    if (res.ok) {
      toast.success(
  ...
  async function doDelete() {
    if (!deleteId) return;
    setPending(true);
    const res = await deleteJenisUnitAction(deleteId);
    setPending(false);
    setDeleteId(null);
    if (res.ok) {
