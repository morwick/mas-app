...
export function UnitDetailView({
  ...
  async function onChangeStatus(next: UnitStatus, reason?: string) {
    setPending(true);
    const res = await changeUnitStatus(unit.id, next, reason);
    setPending(false);
    setStatusOpen(false);
    if (res.ok) {
  ...
  return (
    <div className="grid gap-4 grid-cols-1 lg:grid-cols-[1.6fr_1fr]">
      ...
      {/* Right column */}
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        <DokumenKendaraanCard unit={unit} />
 ...
 */
function DokumenKendaraanCard({ unit }: { unit: Unit }) {
  const items = [
  ...
  return (
    <div className="card card-pad">
      ...
      <div className="caption" style={{ marginBottom: 12 }}>
        {adaIsinya
          ? "Diingatkan 30 hari sebelum habis"
          : "Belum dicatat — isi lewat Edit unit"}
      </div>

      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {items.map((item) => {
          const sisaHari = item.tanggal
            ? Math.floor(
