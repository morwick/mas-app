-- ============================================================================
-- Migration 20260924000027: aktifkan pengguna hanya bila karyawannya Aktif
--
-- * Trigger profiles_aturan_role (versi 000026) + aturan: akun pengguna tidak
--   bisa aktif (dibuat, diaktifkan kembali, atau dipindah karyawan) bila
--   karyawannya berstatus Nonaktif.
-- * status_karyawan_pengguna(): status karyawan tiap akun untuk menu Pengguna
--   (schema hr tidak dibuka di Data API) — superadmin saja.
-- Akun yang SUDAH aktif dengan karyawan nonaktif tidak diubah: aksesnya sudah
-- diblokir sejak migration 000020 (karyawan nonaktif tidak bisa login).
...
CREATE OR REPLACE FUNCTION transport.status_karyawan_pengguna()
RETURNS TABLE (user_id UUID, karyawan_aktif BOOLEAN)
LANGUAGE sql
STABLE
SECURITY DEFINER
SET search_path = transport, extensions
AS $$
  SELECT p.id, transport.karyawan_aktif(p.karyawan_id)
    FROM transport.profiles p
   WHERE transport.is_superadmin() AND p.status = 1;
$$;
REVOKE ALL ON FUNCTION transport.status_karyawan_pengguna() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION transport.status_karyawan_pengguna() TO authenticated, service_role;
