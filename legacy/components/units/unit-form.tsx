...
export function UnitForm({
  ...
  const [form, setForm] = useState({
    ...
    stnk_nomor: initial?.stnk_nomor ?? "",
    stnk_berlaku_sampai: initial?.stnk_berlaku_sampai ?? "",
    kir_nomor: initial?.kir_nomor ?? "",
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    const payload = {
      ...
      stnk_nomor: form.stnk_nomor,
      stnk_berlaku_sampai: form.stnk_berlaku_sampai || null,
      kir_nomor: form.kir_nomor,
      kir_berlaku_sampai: form.kir_berlaku_sampai || null,
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col gap-4 max-w-[640px]">
      ...
      {/* Dokumen kendaraan.
          Dipisah dari data dasar karena isinya punya masa berlaku — begitu
          diisi, unit ini ikut muncul di pengingat dokumen mendekati habis. */}
      <Card>
        <CardHeader
          title="Dokumen kendaraan"
          description="Masa berlaku dipakai untuk pengingat. Kosongkan kalau belum dicatat."
        />
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="No STNK">
            <Input
              placeholder="Nomor STNK"
              value={form.stnk_nomor}
              onChange={(e) => set("stnk_nomor", e.target.value)}
            />
          </Field>
          ...
          <Field label="No KIR" hint="Uji berkala kendaraan">
            <Input
              ...
              value={form.kir_nomor}
              onChange={(e) => set("kir_nomor", e.target.value)}
            />
          </Field>
