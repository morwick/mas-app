...
export async function createUnitAction(input: UnitInput): Promise<ActionResult<{ id: string }>> {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      stnk_nomor: input.stnk_nomor?.trim() || null,
      stnk_berlaku_sampai: input.stnk_berlaku_sampai || null,
      kir_nomor: input.kir_nomor?.trim() || null,
      kir_berlaku_sampai: input.kir_berlaku_sampai || null,
      pajak_berlaku_sampai: input.pajak_berlaku_sampai || null
    })
    .select("id")
    .single();
  if (error) {
...
export async function updateUnitAction(
  ...
  if (input.stnk_nomor !== undefined)
    payload.stnk_nomor = input.stnk_nomor?.trim() || null;
  if (input.stnk_berlaku_sampai !== undefined)
    payload.stnk_berlaku_sampai = input.stnk_berlaku_sampai || null;
  if (input.kir_nomor !== undefined)
    payload.kir_nomor = input.kir_nomor?.trim() || null;
  if (input.kir_berlaku_sampai !== undefined)
    payload.kir_berlaku_sampai = input.kir_berlaku_sampai || null;
  if (input.pajak_berlaku_sampai !== undefined)
    payload.pajak_berlaku_sampai = input.pajak_berlaku_sampai || null;
