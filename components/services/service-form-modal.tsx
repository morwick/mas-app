    ...
    async function fetchLookup() {
      try {
        const res = await fetch(
          `/api/units/${unitId}/mileage-at?date=${form.tanggal}`,
          { cache: "no-store" }
        );
        ...
        const next: LookupState = {
          ...
          akumulasiKm:
            typeof body.akumulasi_km === "number" ? body.akumulasi_km : null,
          kmAfterTarget:
            typeof body.km_after_target === "number"
              ? body.km_after_target
              : null,
          dataQuality: body.data_quality ?? null,
          missingDays: Array.isArray(body.missing_days) ? body.missing_days : [],
          error: null
        };
        setLookup(next);
        if (
